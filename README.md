...
Line counts across git-tracked source (comments and blanks included,
`__pycache__` excluded). Regenerate with `bash scripts/lang-stats.sh` after any
change that adds, removes, or resizes source files.

<!-- LANG-STATS:START (auto-generated by scripts/lang-stats.sh — do not edit by hand) -->

| Type | Lines | Share | In GitHub bar? |
| --- | ---: | ---: | :---: |
| Markdown | 1485 | 31.5% | — |
| YAML | 1228 | 26.0% | — |
| Python | 1218 | 25.8% | ✅ |
...
> which counts only `programming`/`markup` file types. SQL, YAML and JSON
> are `data`, and Markdown is `prose` — all excluded — so the bar
> renormalizes over the ✅ rows only and hides most of this project's actual
> transformation logic (dbt SQL) and configuration.

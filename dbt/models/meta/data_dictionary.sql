...
FROM VALUES
    ...
    ('staging.stg_orders',        'status',        'Order lifecycle status: CREATED, PAID, PREPARING, SHIPPED, DELIVERED, CANCELLED, RETURNED or REFUNDED.'),
    ...
    ('lakehouse.silver.core_orders',      'status',       'Order lifecycle status: CREATED, PAID, PREPARING, SHIPPED, DELIVERED, CANCELLED, RETURNED or REFUNDED.'),
    ...
    ('lakehouse.silver.core_orders',      'paid_amount',  'total_amount when status is a revenue state (PAID/PREPARING/SHIPPED/DELIVERED), else 0. Revenue basis for the marts.'),
    ('lakehouse.silver.core_orders',      'is_cancelled', '1 when status=CANCELLED (unpaid cancellation), else 0.'),
    ('lakehouse.silver.core_orders',      'is_returned',  '1 when status=RETURNED, else 0.'),
    ('lakehouse.silver.core_orders',      'is_refunded',  '1 when status=REFUNDED (sale reversed), else 0.'),
    ...
    ('lakehouse.gold.mart_daily_revenue',     'total_revenue',    'Sum of paid_amount (revenue states: PAID/PREPARING/SHIPPED/DELIVERED) on that day.'),
    ...
    ('lakehouse.gold.mart_sales_by_category', 'total_orders',     'Distinct revenue-state (PAID/PREPARING/SHIPPED/DELIVERED), non-deleted orders containing this category.'),
